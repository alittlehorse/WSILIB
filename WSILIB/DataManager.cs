       ...
       internal static Frame SelectFrameByOffset(int x, int y,int w, int h,int c)
        {
            var tmp = Sqlhelper.Exesql($"SELECT * From Frame,Image WHERE " +
                ...
                $"and Frame.SOPInstanceUID = (SELECT Image.SOPInstanceUID WHERE Image.L = {c}) ");
            if(tmp.Rows.Count!=0)
            {
        ...
        internal static String[] GetSOPInstanceUIDsInLevel(int level)
        {
            ...
            var table = Sqlhelper.Exesql($"SELECT SOPInstanceUID FROM Image WHERE L = {level}");
        ...
        {
            var tmp = Sqlhelper.Exesql($"Select * FROM Local_Frame WHERE SOPInstanceUID=\'{sOPInstanceUID}\' and FramIndex = {frameIndex}");
            if (tmp.Rows.Count!=0)
            {
        ...
        {
            Sqlhelper.ExecuteNonQuery($"INSERT INTO Image(SOPInstanceUID,R,C,TotalPixelMatrixColumns,TotalPixelMatrixRows,L) Values(\'{SOPInstanceUID}\',{Rows},{Columns},{TotalPixelMatrixColumns},{TotalPixelMatrixRows},{Level})");            
        }
        ...
        internal static void InitLevelInImage()
        {
            ...
            {                
                Sqlhelper.ExecuteNonQuery($"UPDATE Image SET L = {c--} WHERE TotalPixelMatrixRows = {MaxSize}");
                MaxSize = MaxSize/2;
            }
        }
        ...
        {
            Sqlhelper.ExecuteNonQuery($"INSERT INTO Local_Frame(SOPInstanceUID,FramIndex,LocalFilePath,LocalFrameIndex) Values(\'{oldSOPInstancUID}\',{v},\'{path}\',{i})");
        }
